define(function (require) {
	...
	var	app = require('durandal/app'),
		...
		key = {	name: ko.observable(),
				...
				relevantTaxa: ko.pureComputed(function() {
					return _.uniq(_.filter(key.taxa(), function(taxon) {return taxon.reasonsToDrop === 0;}), function(taxon) {return taxon.id + taxon.subset;});
				}),
				irrelevantTaxa: ko.pureComputed(function() {
					return _.uniq(_.filter(key.taxa(), function(taxon) {
						return taxon.reasonsToDrop !== 0 && !(_.some(key.relevantTaxa(), {'id': taxon.id, 'subset': taxon.subset}));
					}), function(taxon) {return taxon.id + taxon.subset;});
				}),
				
				remaining: ko.pureComputed(function(){return key.relevantTaxa().length;}),
				removed: ko.pureComputed(function(){return _.flatten(removedTaxa()).length;}),
				commonTaxonomy: ko.observable([]),
				foundTaxonomy: ko.pureComputed(function() {
					var array = _.map(_.pluck(key.relevantTaxa(), 'taxonObject.AcceptedName.higherClassification'), function(tax) {return _.slice(tax, key.commonTaxonomy().length);});
					return _.filter(_.first(array), function(firstItem) {return _.every(_.rest(array), function(otherArray) {return _.some(otherArray, function(otherItem) {return _.isEqual(firstItem, otherItem);});});});
				}),
				lastCommon: function() {
				...
				taxaList: ko.pureComputed(function() {
					var unique = _.uniq(_.clone(key.relevantTaxa(), true), function(taxon){return taxon.id;});
				...
				droppedTaxa: ko.pureComputed(function() {
					var unique = _.uniq(_.clone(key.irrelevantTaxa(), true), function(taxon){return taxon.id;});
					_.forEach(unique, function(taxon) {
				...
				focus: ko.pureComputed(function() {
					if(key.characters_all().length > 0 && (key.characters_checked().length > 0 && key.remaining() > 1 && _.some(_.last(key.characters_checked()).states, function(state) {return state.status() === null;}))) {
				...
				characters_checked: ko.pureComputed(function() {
					return _.filter(key.characters(), function(character){return character.evaluate() && character.checked() && character.relevance() === 0;}).sort(function(a,b) {return a.timestamp() - b.timestamp();});
				}),
				characters_unanswered: ko.pureComputed(function() {
					var array = _.filter(key.characters(), function(character){return !character.skipped() && character.relevance() === 1 && character.evaluate();});
					return array.sort(function(a,b) {
						if (a.timestamp() != b.timestamp()) {return b.timestamp() - a.timestamp();}
						if (a.sort != b.sort) {return a.sort - b.sort;}
						return a.skewness() - b.skewness();
				});}),
				characters_skipped: ko.pureComputed(function() {return _.filter(key.characters(), function(character){return character.skipped() && !character.checked() && character.relevance() > 0 && character.evaluate();});}),
				characters_hidden: ko.pureComputed(function() {return _.filter(key.characters(), function(character){return !character.skipped() && (!character.evaluate() || (character.relevance() > 1));});}),
		...
		parseCSV = function (array) {
			...
			for (var i = headerRow+1; i < array.length; i++) {
				...
				if(characterName) {
					characters.push({id: i, string: characterName, sort: sort, description: description, rule: rule, multistate: multi, states: []});
				}
									
				characters[characters.length-1].states.push({parent: characters[characters.length-1].id, id: i, string: stateName, ref: ref, media: media});
		...
		fillKey = function (taxa, characters) {
					...
					$.getJSON("http://data.artsdatabanken.no/Api/Taxon/" + taxon.id, function(data) {
						...
						if(taxon.taxonObject) {
							if(taxon.taxonObject.AcceptedName) {
								...
								taxon.taxonObject.AcceptedName.higherClassification.push({taxonRank: taxon.taxonObject.AcceptedName.taxonRank, scientificName: taxon.taxonObject.AcceptedName.scientificName});
							}
						...
						if(taxon.description === null) {
							taxon.description = _.some(taxon.taxonObject.scientificNames, function(sn) {
								return _.some(sn.dynamicProperties, function(dp) {return dp.Name === "Description";})
							});
						}
			...
			$.when.apply($, gettingTaxa).then(function() {
				key.taxa(taxa.sort(function(a,b) {return a.sort - b.sort;}));
				var array = _.pluck(key.taxa(), 'taxonObject.AcceptedName.higherClassification');
				key.commonTaxonomy(_.filter(_.first(array), function(firstItem) {return _.every(_.rest(array), function(otherArray) {return _.some(otherArray, function(otherItem) {return _.isEqual(firstItem, otherItem);});});}));

				
				//~ fetch abundances from the API
				_.each(_.uniq(taxa, function(taxon){return taxon.id;}), function(taxon) {
					gettingAbundances.push(function(taxon) {
						...
						$.getJSON("http://pavlov.itea.ntnu.no/artskart/Api/Observations/list/?pageSize=0&taxons[]=" + taxon.id, function(data) {
							_.forEach(_.filter(taxa, function(t) {return t.id === taxon.id;}), function(taxon) {
				...
				$.when.apply($, gettingAbundances).then(function() {
					key.taxa(taxa.sort(function(a,b) {
						if (a.sort != b.sort) {return a.sort - b.sort;}
						if (a.abundance != b.abundance) {return b.abundance - a.abundance;}
						return 0;
					}));
				});
			});
			...
			_.forEach(characters, function(character) {
				...
				character.checked = ko.pureComputed(function() {
					return _.some(character.states, function(state) {return state.checked() !== null;});
				});
				...
				character.evaluate = ko.pureComputed(function() {
					...
					function evaluateState(ref) {
						return (_.find(_.find(characters, function(char) {return _.some(char.states, function(s){return s.ref === ref;});}).states, function(state) {return state.ref === ref;})).status() === 1;
					}
				});
				...
				_.each(character.states, function(state) {					
					state.zeroes =  ko.pureComputed(function() {
						return _.filter(key.relevantTaxa(), function(taxon) {return _.some(taxon.stateValues, {state: state.id, value: 0});}).length;
					});
					
					state.status = ko.pureComputed(function() {
						if(state.checked() !== null) return state.checked();
						if(character.checked() && !character.multistate) {
							if(_.some(character.states, function(state) {return state.checked() === 1;})) {
								return 0;
							}
						...
						if((_.filter(key.relevantTaxa(), function(taxon) {return _.some(taxon.stateValues, {state: state.id, value: 1});})).length === key.relevantTaxa().length) return 1;
						return null;
					});
					...
					state.relevance = ko.pureComputed(function() {						
						...
						if(key.relevantTaxa().length === 1 || state.status() !== null) {return 0;}

						//~ otherwise, as long as it's never totally unknown (even when not distinctive), it's always a valid question
						var haveValues = _.filter(key.relevantTaxa(), function(taxon) {return _.some(taxon.stateValues, {state: state.id});});
						if(key.relevantTaxa().length === haveValues.length) {return 1;}
				...
				character.relevance = ko.pureComputed(function() {
					...
					var stateRelevances = _.filter(_.map(character.states, function(state){return state.relevance();}), function(n){ return n > 0;});
				...
				character.skewness = ko.pureComputed(function() {
					...
					var relevantStates = _.filter(character.states, function(state){return state.status() === null;}),
						perfect = key.relevantTaxa().length * (1-(1 / relevantStates.length));

					return Math.sqrt(_.reduce(relevantStates, function(total, state){ return total + ((state.zeroes() - perfect) * (state.zeroes() - perfect));}, 0) / relevantStates.length);
				});
			});
		...
		setState = function (id, parent, value) {
			var character = _.find(key.characters(), function(char) {return char.id === parent;}),
				state = _.find(character.states, function(state) {return state.id === id;}),
				oldValue = state.checked();
			state.checked(value);
			character.timestamp(Date.now());
			
			_.each(key.taxa(), function(taxon) {
				if(value === 1 && _.find(taxon.stateValues, {'state': state.id, 'value': 0})) taxon.reasonsToDrop = taxon.reasonsToDrop+1;
				else if(value === 0 && _.find(taxon.stateValues, {'state': state.id, 'value': 1})) taxon.reasonsToDrop = taxon.reasonsToDrop+1;
				else if(value === null && _.find(taxon.stateValues, {'state': state.id, 'value': 1-oldValue})) taxon.reasonsToDrop = taxon.reasonsToDrop-1;
			});
			key.taxa.valueHasMutated();
		},
	...
	return {
		...
		removeSelected: function(taxon) {
			...
			if(_.uniq(_.clone(key.relevantTaxa(), true), function(t){return t.id;}).length === 1) {
				var removing = _.pluck(_.filter(key.taxa(), function(t){return t.id === taxon.id && t.subset === taxon.subset}), 'index');
			}
			//~ otherwise remove all with that id
			else {
				var removing = _.pluck(_.filter(key.taxa(), function(t){return t.id === taxon.id}), 'index');
			}
		...
		undoRemoval: function(taxon) {
			if(taxon.index) {
				...
				var deletion = _.find(removedTaxa(), function(l){return _.includes(l, taxon.index);});
				_.forEach(deletion, function(i){
					dropTaxon([i], -1);
				});
				removedTaxa(_.reject(removedTaxa(), function(t){ return t[0] === taxon.index;}));
			}
			else
			...
			if(!taxon.widgetHtml) {
				if(taxon.description === true) {
					...
					_.forEach(taxon.taxonObject.scientificNames, function(sn) {
						_.forEach(_.filter(sn.dynamicProperties, function(dp) {return dp.Name === "Description";}), function(dp) {
		...
		skipCharacter: function(character) {
			_.find(key.characters(), function(char) {return char.id === character.id;}).skipped(true);
		},
