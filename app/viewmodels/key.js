...
function (app, ko, http, router, _) {
  ...
  var URL_API_NAME = URL_ARTSDATABANKEN + 'Api/Taxon/ScientificName/';
  ...
  var key = {
    ...
    irrelevantTaxa: ko.pureComputed(function () {
      return _.uniq(_.filter(key.taxa(), function (taxon) {
        ...
        return taxon.taxonId + taxon.subset + taxon.morph;;
      });
    }),

    remainingSubsets: ko.pureComputed(function () {
      if(key.usesSubsets)
      return _.uniq(key.relevantTaxa(), function(taxon) {return taxon.taxonId + taxon.subset;}).length;
      else
      return _.uniq(key.relevantTaxa(), function(taxon) {return taxon.taxonId;}).length;
    }),
    ...
    removed: ko.pureComputed(function () {
      var uniqueSubsets = _.uniq(_.cloneDeep(removedTaxa()), function (taxon) {
        return taxon.taxonId + taxon.subset;
      });
    ...
    taxaList: ko.pureComputed(function () {
      ...
      if(key.usesMorphs) {
        _.forEach(uniqueTaxa, function(t){
          if(_.some(key.relevantTaxa(), function(r) {return r.taxonId == t.taxonId && r.morph != t.morph;})) {
            t.morph = null;
          }
        });
      }
      ...
      for (i = 0; i < uniqueTaxa.length; i++)
      {
        ...
        taxon.subset = (_(_.pluck(_.filter(uniqueSubsets, function(t) {return t.taxonId === taxon.taxonId && t.subset;}), 'subset')).toString()).replace(",","/");
      }
  ...
  parseCSV = function (array) {
    ...
    taxonScientificNameIdRow = taxonHeaders.indexOf('ScientificNameID'),
    ...
    for (var i = headerColumn + 1; i < array[0].length; i++) {
      taxa.push({
        taxonId: (taxonIdRow > -1 && array[taxonIdRow][i] && $.isNumeric(array[taxonIdRow][i]) ? array[taxonIdRow][i] : null),
        scientificNameId: (taxonScientificNameIdRow > -1 && array[taxonScientificNameIdRow][i] && $.isNumeric(array[taxonScientificNameIdRow][i]) ? array[taxonScientificNameIdRow][i] : null),
        index: i,
          ...
          for (var i = 0; i < character.states().length; i++)
          {
            if(_.some(key.relevantTaxa(), function(t) {
              return _.some(key.relevantTaxa(), function(r) {
                return (r.taxonId == t.taxonId && r.subset == t.subset && !_.isEqual(_.find(t.stateValues, {'state': character.states()[i].id}), _.find(r.stateValues, {'state': character.states()[i].id})));
              });
            })) {
              return 1;
            }
  ...
  return {
    ...
    removeSelected: function (taxon) {
      ...
      if (key.usesSubsets && _.uniq(_.cloneDeep(key.relevantTaxa()), function (t) {
        ...
        var removing = _.pluck(_.filter(key.taxa(), function (t) {
          return t.taxonId === taxon.taxonId && t.subset === taxon.subset
        }), 'index');
      }
      ...
      //~ otherwise remove all with that id
        ...
        var removing = _.pluck(_.filter(key.taxa(), function (t) {
          return t.taxonId === taxon.taxonId
        }), 'index');
      }
    ...
    showTaxonPage: function (t) {
      ...
      if (taxon.taxonId > 0) {
        $.get(URL_WIDGETS + '/Taxon/' + taxon.taxonId, function (data) {
          key.widgetHtml(widgetHtml(URL_WIDGETS+ '/Taxon/' + taxon.taxonId));
          key.widgetLink(URL_TAXON + taxon.taxonId);
        });
      }
    },
